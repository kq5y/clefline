...
export class OpenSheetMusicDisplay {
    ...
    /**
     * Async version of render that yields to main thread periodically.
     * This prevents UI freezing for large scores.
     */
    public async renderAsync(
        onProgress?: (phase: string, current: number, total: number) => void
    ): Promise<void> {
        if (!this.graphic) {
            throw new Error("OSMD: load() needs to be called before renderAsync()");
        }
        this.drawer?.clear();
