...
export class VexFlowMusicSheetDrawer extends MusicSheetDrawer {
    ...
    public async drawSheetAsync(
        ...
        // Count total systems for progress
        let totalSystems: number = 0;
        let currentSystem: number = 0;
        const pagesToDraw: number = Math.min(graphicalMusicSheet.MusicPages.length, this.rules.MaxPageToDrawNumber);
        for (let i: number = 0; i < pagesToDraw; i++) {
            totalSystems += graphicalMusicSheet.MusicPages[i].MusicSystems.length;
        }

        // Draw systems progressively (using original drawMusicSystem to maintain DOM structure)
        this.pageIdx = 0;
        for (let i: number = 0; i < pagesToDraw; i++) {
